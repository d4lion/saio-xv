...
export default function WhatsAppButton() {
  ...
  const defaultMessage = "Hola equipo SAIO XV, me gustaria obtener informacion sobre el evento de este 15 y 16 de octubre y conocer las opciones disponibles para adquirir mis boletas.";
  const whatsappUrl = `https://wa.me/${phone}?text=${encodeURIComponent(defaultMessage)}`;

  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-row-reverse items-end gap-3 pointer-events-auto select-none">
      {/* Floating Button */}
      <div className="relative group">
        {/* Outer glowing pulse ring */}
        <div className="absolute inset-0 rounded-full bg-emerald-500/40 blur-md animate-ping pointer-events-none" />
      ...
      <AnimatePresence>
        {showTooltip && (
          <motion.div
            ...
            className="hidden sm:flex items-center gap-2.5 bg-[#0a151d]/90 backdrop-blur-md text-white p-3 pr-3.5 rounded-2xl border border-emerald-500/30 shadow-2xl shadow-emerald-900/30 max-w-xs relative group/tooltip"
          >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 text-xs text-emerald-50 font-sans leading-tight hover:text-emerald-300 transition-colors"
            >
              <span className="font-bold text-white block mb-0.5">¿Preguntas sobre el evento?</span>
              <span className="text-[11px] text-gray-300">Chatea con el equipo SAIO XV para comprar tu boleta.</span>
            </a>
